...
class Blockchain:
    ...
    def is_chain_valid(self, chain):
        '''check if all the blockchain is valid'''
        previous_block = chain[0]
        block_index = 1
        #loop over every block on chain
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Vinicius', amount = 1)
...
def add_transaction():
    ...
    response = {'message': f'Transaction will be inserted in block {index}'}
...
def replace_chain():
    if blockchain.replace_chain():
        ...
        response = {'message': 'Chain OK. Nothing to update',
